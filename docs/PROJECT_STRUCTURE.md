...
- `mad_colab_pkg/image_utils.py` — document normalization, perspective transform, CLAHE and ROI crops.
...
- `mad_colab_pkg/face_service.py` — DeepFace availability checks, embeddings and cosine distance.
...
- `data/passport-2000.csv` — small public synthetic sample for OCR-line dataset generation.
...
- `sql/supabase_schema_demo_insecure.sql` — demo-only schema variant with RLS disabled.

## Notebook

- `notebooks/mad_identity_verification_colab.ipynb` — Colab demo notebook that reads Supabase credentials from Secrets or environment variables.
